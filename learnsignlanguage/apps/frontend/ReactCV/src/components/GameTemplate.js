...
// import { createScore, getScore } from "../redux/score";
...
//   //it is use for update the score
//   const updateScoreSheet = useCallback(() => {
//     if (scoreRef.current > 0) {
//       const today = new Date().toLocaleDateString('en-CA');
//       const title = location.pathname.split('/').filter(Boolean)[0]
//       const currentScore = scoreRef.current;
//       let newScoreSheet = scoreSheetRef.current;
//       newScoreSheet.push({ title: title, date: today, score: currentScore });
//       dispatch(createScore({ title: title, date: today, score: currentScore, userId: localStorage.getItem('userId') }))
//       dispatch(getScore(localStorage.getItem('userId')))
...
//                     Indian sign language alphabet guide
//                   </span>
//                 </button>
//               </StyledTogglePanel>
//               <StyledAslModal
...
//                 aria-describedby="A short guide to Indian Sign Language Alphabets"
//               >
//                 <StyledAslImg src={aslImg} alt="Indian Sign Language Guide" />
//               </StyledAslModal>
//             </StyledBoyContainer>
//           </StyledLeftPanel>
//           <StyledRightPanel>
...
import { CircularProgress, Slider, Switch } from "@mui/material";
import * as tf from "@tensorflow/tfjs";
import aslImg from "../assets/images/ASL_Alphabet.png";
import boyImg from "../assets/images/boy-hand.png";
import tick from "../assets/images/checked.png";
import studyIcon from "../assets/images/notebook.png";
import DSButton from "../components/DSButton";
import TextBubble from "../components/TextBubble";
import { drawRect } from "../utilities";
...
function GameTemplate({
  ...
  const [score, setScore] = useState(0);
  const [threshold, setThreshold] = useState(0.7);
  const { currentScore: reduxScores, loading: scoresLoading } = useSelector((state) => state.score);
  ...
  useEffect(() => {
    ...
    if (userId) {
      dispatch(getScore(userId)).then((response) => {
        if (response.payload) {
          setScoreSheet(response.payload);
          scoreSheetRef.current = response.payload;
        }
      });
    }
  }, [dispatch]);
  ...
  const onNextLetter = useCallback(() => {
    ...
    if (
      ...
      scoreRef.current += 1;
      setScore(scoreRef.current);
      // Update score sheet immediately when a word is completed
      const today = new Date().toLocaleDateString('en-CA');
      const gameTitle = location.pathname.split('/').filter(Boolean)[0];
      const newScore = { title: gameTitle, date: today, score: scoreRef.current };
      
      const newScoreSheet = [...scoreSheetRef.current, newScore];
      setScoreSheet(newScoreSheet);
      scoreSheetRef.current = newScoreSheet;
      ...
      const userId = localStorage.getItem('userId');
      if (userId) {
        dispatch(createScore({ ...newScore, userId })).then(() => {
          // After creating score, fetch updated scores
          dispatch(getScore(userId)).then((response) => {
            if (response.payload) {
              setScoreSheet(response.payload);
              scoreSheetRef.current = response.payload;
  ...
  }, [handleChooseWord, wordBank, dispatch, location]);
    ...
    <StyledWrapper>
      <StyledAppContainer>
        ...
        <StyledContentBody>
          <StyledLeftPanel>
            ...
            <StyledBoyContainer>
              <StyledBoyImg alt="Boy raising hand" src={boyImg} />
              <StyledTogglePanel>
                <StyledSwitchGroup isHidden={isWordMode}>
                  <StyledSwitchLabel>Show answers:</StyledSwitchLabel>
                  <Switch
      ...
      <StyledAppContainer>
        ...
        {scoresLoading ? (
          ...
          <StyledTable>
            <YourScore currentScore={scoreSheet} />
          </StyledTable>
        )}
      </StyledAppContainer>
    </StyledWrapper>
